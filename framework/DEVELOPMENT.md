...
- **移动模式**：单击分组标题（整组）或零件名（单件）进入；移动条选**平面** XY/XZ/YZ（唯一、
  互斥），左键在平面内拖拽（屏幕位移→相机右/上→投影到两轴）；右键/Esc/退出按钮取消并还原；
  多选：移动模式下继续点其他标题/零件即追加；选集行高亮+3D emissive 高亮+参考网格（随平面转向）
- **撤销**：底部"撤销"按钮，记录移动（网格位置 before/after）与零件增删（文件文本快照），上限 100 步
- **切片全局化**：solidTarget=所有文件可见实体的并集；隐藏件不参与；剖切面作用于全部模型材质
